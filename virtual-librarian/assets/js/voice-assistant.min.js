...
document.addEventListener('DOMContentLoaded', () => {
    ...
    if (typeof vl_voice_data === 'undefined' || !vl_voice_data.enabled) {
        return;
    }
    ...
    // --- 24-HOUR TEST LOGIC ---
    let hasAccess = vl_voice_data.is_logged_in || !vl_voice_data.test_mode;
            ...
            if (!localStorage.getItem('cl_voice_test_feedback_done') && isMobileOrKiosk) {
                ...
                if (feedbackModal) {
                    ...
                    form.addEventListener('submit', (e) => {
                        ...
                        formData.append('nonce', vl_voice_data.ai_nonce);

                        fetch(vl_voice_data.ajax_url, { method: 'POST', body: formData })
                            .then(res => res.json())
                            .then(data => {
    ...
    function setBestRussianVoice(utterance, voices) {
        ...
        if (ruVoices.length > 0) {
            ...
            utterance.pitch = parseFloat(vl_voice_data.voice_pitch) || 1.1; // Slightly higher pitch for softer tone
            utterance.rate = parseFloat(vl_voice_data.voice_rate) || 0.95;  // Slightly slower for more natural human pacing
        }
    }
        ...
        if (mapModal && mapTitle && mapIframe) {
            ...
            if (showAll && customMapContainer) {
                ...
                if (customMapContainer.innerHTML.trim() === '') {
                    ...
                    fetch(vl_voice_data.ajax_url, {
                        method: 'POST',
                        headers: {
                            'Content-Type': 'application/x-www-form-urlencoded',
                        },
                        body: new URLSearchParams({
                            action: 'vl_get_map_shortcode',
                            nonce: vl_voice_data.ai_nonce
                        })
                    })
    ...
    function processCommand(command) {
        ...
        const addr = vl_voice_data.branch_addresses || {};
        ...
        if (vl_voice_data.custom_commands && vl_voice_data.custom_commands.length > 0) {
            for (let i = 0; i < vl_voice_data.custom_commands.length; i++) {
                const customCmd = vl_voice_data.custom_commands[i];
        ...
        if (cmd.includes('открой последние новости') || cmd.includes('открой последнюю новость')) {
            ...
            if (firstNewsLink && firstNewsLink.href) {
                ...
                window.location.href = vl_voice_data.home_url + '/?news_archive=true#primary';
            }
            return;
        }
        ...
        if (cmd.includes('афиша новостей') || cmd.includes('афиша') || cmd.includes('мероприятия')) {
            ...
            if (document.getElementById('afisha')) {
                ...
                window.location.href = vl_voice_data.home_url + '/#afisha';
            }
            return;
        }
        ...
        if (cmd.includes('на главную') || cmd.includes('главная страница')) {
            ...
            window.location.href = vl_voice_data.home_url + '/#primary';
            return;
        }
        ...
        if (cmd.includes('открой контакты') || cmd.includes('раздел контакты')) {
            ...
            if (contactLink && contactLink.href) {
                 ...
                 window.location.href = vl_voice_data.home_url + '/#branches';
            }
            return;
        }
