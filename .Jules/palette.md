...
## 2024-05-24 - Screen Reader Announcements
**Learning:** Using browser `alert()` to announce state changes (like accessibility mode toggles) blocks the main thread and creates a poor UX for both sighted and screen-reader users.
**Action:** Always use a visually hidden `aria-live="polite"` region to gracefully announce state changes without interrupting the user's flow.
