...
## 2024-06-24 - Material Symbols Accessibility in Localized Apps
**Learning:** Using Material Symbols (e.g., `<span class="material-symbols-outlined">chat</span>`) without `aria-hidden="true"` inside icon-only buttons causes screen readers to read the literal English ligature text (e.g., "chat") instead of the localized Russian action, confusing users.
**Action:** Always pair Material Symbols with `aria-hidden="true"` when used as icons inside buttons or links, and provide a translated `aria-label` and `title` on the parent interactive element.
