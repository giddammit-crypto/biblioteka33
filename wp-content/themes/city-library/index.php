...
<div class="w-full max-w-[95%] mx-auto px-4 sm:px-6 lg:px-8 py-12">
    ...
    <div class="flex flex-col lg:flex-row gap-8 relative items-start">
        ...
        <!-- Left Sidebar (30%) -->
        <aside id="left-sidebar-wrapper" class="w-full lg:w-[30%] shrink-0 transition-all duration-500 ease-in-out origin-left relative group">
            <div class="bg-slate-50 dark:bg-slate-900/50 p-6 rounded-3xl border border-slate-100 dark:border-slate-800 h-full">
                <?php if (is_active_sidebar('left-sidebar')) : ?>
                    <?php dynamic_sidebar('left-sidebar'); ?>
                <?php else : ?>
                    <div class="text-center text-slate-400 py-10">
                        <span class="material-symbols-outlined text-4xl mb-2">widgets</span>
                        <p><?php _e('Добавьте виджеты в "Left Sidebar"', 'city-library'); ?></p>
                    </div>
                <?php endif; ?>
            </div>
        </aside>
        ...
        <!-- Main Content (70%) -->
        <div id="primary" class="w-full lg:w-[70%] grow transition-all duration-500 ease-in-out relative">

            <!-- Toggle Button -->
            <div class="absolute -left-5 top-8 z-20 hidden lg:block">
                <button id="sidebar-toggle" class="bg-primary hover:bg-yellow-500 text-white rounded-full p-2 shadow-lg transition-colors flex items-center justify-center transform hover:scale-110" aria-label="Toggle Sidebar" aria-expanded="true">
            ...
            <div class="content-area bg-slate-50 dark:bg-slate-900/50 p-8 rounded-3xl border border-slate-100 dark:border-slate-800" style="background-image: url('data:image/svg+xml,%3Csvg width=\'52\' height=\'26\' viewBox=\'0 0 52 26\' xmlns=\'http://www.w3.org/2000/svg\'%3E%3Cg fill=\'none\' fill-rule=\'evenodd\'%3E%3Cg fill=\'%239C92AC\' fill-opacity=\'0.1\'%3E%3Cpath d=\'M10 10c0-2.21-1.79-4-4-4-3.314 0-6-2.686-6-6h2c0 2.21 1.79 4 4 4 3.314 0 6 2.686 6 6 0 2.21 1.79 4 4 4 3.314 0 6 2.686 6 6 0 2.21 1.79 4 4 4v2c-3.314 0-6-2.686-6-6 0-2.21-1.79-4-4-4-3.314 0-6-2.686-6-6zm25.464-1.95l8.486 8.486-1.414 1.414-8.486-8.486 1.414-1.414z\' /%3E%3C/g%3E%3C/g%3E%3C/svg%3E');">
                ...
                <?php if (have_posts()) : ?>
                    <div id="posts-container" class="grid grid-cols-1 sm:grid-cols-2 md:grid-cols-2 lg:grid-cols-3 gap-6">
                        <?php
                        while (have_posts()) :
                            the_post();
