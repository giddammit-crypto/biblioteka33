...
<footer class="bg-secondary text-white py-16 bg-pattern-white <?php echo city_library_get_animation_class(); ?>" style="background-color: <?php echo esc_attr(get_theme_mod('footer_bg_color', '#1A3C34')); ?>; color: <?php echo esc_attr(get_theme_mod('footer_text_color', '#FFFFFF')); ?>;">
    <div class="w-full max-w-[95%] lg:max-w-[90%] xl:max-w-[80%] mx-auto px-4 sm:px-6 lg:px-8 grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-12">
    ...
    <div class="w-full max-w-[95%] lg:max-w-[90%] xl:max-w-[80%] mx-auto px-4 sm:px-6 lg:px-8 mt-16 pt-8 border-t border-white/10 flex flex-col md:flex-row justify-between items-center text-xs text-slate-500">
...
<button id="back-to-top" class="hidden fixed bottom-24 lg:bottom-8 right-8 w-14 h-14 bg-primary hover:bg-yellow-600 text-secondary rounded-full shadow-2xl flex items-center justify-center transition-transform hover:scale-110 z-50" aria-label="<?php esc_attr_e('Вернуться наверх', 'city-library'); ?>">
